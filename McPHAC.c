...
int main( int argc, char *argv[] ) {
    ...
    // Set variables given through input
    if ( argc != 15+1 ) {
    ...
    nmu = (USEINTERPOL) ? 1 : NMU; // No angle dependence in the temperature correction procedure,
				   // so only one angle point is needed if rad. trans uses separate column array
    nfreq = NFREQ;
    ...
    do {
		...
		for (k=0; k<NFREQ; k++) {
			...
			for (j=0; j<NMU; j++) {
			    ...
			    InvertTridiagonalMatrix(M.TInverse,M.T,ndepthsnu);
		...
		fclose(fp8);
            ...
            // Temperature correction proccedure
		...
		for (k=0; k<NFREQ; k++) {
		    ...
		    fprintf(stderr,"%d...", k);
		    InvertTridiagonalMatrix(M.TInverse,M.T,ndepthst);
