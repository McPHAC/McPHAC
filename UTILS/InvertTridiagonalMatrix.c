...
void InvertTridiagonalMatrix(double** ans, double** A, int n) {
	...
	// TODO: Note that forming an explicit inverse for a tri-diagonal matrix is
	// inefficient, so rather than the present method of computing the inverse and
	// then doing a matrix--vector multiply, it should be faster to do a direct
	// tri-diagonal solve for the vector wherever needed, so it might be
	// worthwhile to change this
